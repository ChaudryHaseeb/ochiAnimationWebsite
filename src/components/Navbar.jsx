...
function Navbar() {
  ...
  return (
    <div className={`fixed z-[999] w-full bg-gradient-to-r from-blue-500 px-14 py-8 font-["Neue Montreal"] flex justify-between items-center ${visible ? '' : 'hidden'}`}>
        <div className="logo">
        ...
        <div className="links flex gap-10">
            {["Service","Our work", "About us","Insights","Contact us"].map((item,index)=>(
                <a key={index} className={`text-lg cursor-pointer capitalize font-light ${index === 4 && "ml-48"}`}>{item}</a>
            ))}
        </div>
